...
{
    [Export(typeof(AfrpFleetTrendAllViewModel))]
    [PartCreationPolicy(System.ComponentModel.Composition.CreationPolicy.Shared)]
    public class AfrpFleetTrendAllViewModel : NotificationObject, IPartImportsSatisfiedNotification, IConfirmNavigationRequest
    {
        public AfrpFleetTrendAllViewModel()
        {
        ...
        private readonly UniCloud.AFRP.Views.AfrpFleetTrendAllView _view = ServiceLocator.Current.GetInstance<UniCloud.AFRP.Views.AfrpFleetTrendAllView>();
        ...
        private RadGridView planDetailGridview, ExportRadgridview; //初始化RadGridView
        ...
        private void ViewModelInitializer()
        {
            ...
            //控制界面起止时间控件的字符串格式化
            this.planDetailGridview = _view.planDetailGridview as RadGridView;
            ...
            if (menu != null && menu.Header.ToString() == "导出源数据")
            {
                ...
                if (menu.Name == "LineGridData")
                {
                    if (_service.CurrentAirlines.SubAirlines != null && _service.CurrentAirlines.SubAirlines.Any(p => p.SubType == 1))
                    {
                        ...
                        columnsList.Add("AircraftAmount", "期末飞机数(子)");
                        columnsList.Add("AircraftAmount1", "期末飞机数");
                        ExportRadgridview = ImageAndGridOperation.CreatDataGridView(columnsList, FleetAircraftTrendLineCollection, "SubFleetTrendAll");
                    }
                    else
                    {
                        //创建RadGridView
                        ...
                        columnsList.Add("DateTime", "时间点");
                        columnsList.Add("AircraftAmount1", "期末飞机数");
                        ExportRadgridview = ImageAndGridOperation.CreatDataGridView(columnsList, FleetAircraftTrendLineCollection, "FleetTrendAll");
                    }
                    i = 1;
                ...
                else if (menu.Name == "BarGridData")
                {
                    if (_service.CurrentAirlines.SubAirlines != null && _service.CurrentAirlines.SubAirlines.Any(p => p.SubType == 1))
                    {
                        ...
                        Dictionary<string, string> columnsList = new Dictionary<string, string>();
                        columnsList.Add("DateTime", "时间点");
                        columnsList.Add("AircraftAmount", "飞机净增数(子)");
                        columnsList.Add("AircraftAmount1", "飞机净增数");
                        ExportRadgridview = ImageAndGridOperation.CreatDataGridView(columnsList, FleetAircraftTrendBarCollection, "SubFleetTrendAll");
                    }
                    else
                    {
                        //创建RadGridView
                        ...
                        columnsList.Add("DateTime", "时间点");
                        columnsList.Add("AircraftAmount1", "飞机净增数");
                        ExportRadgridview = ImageAndGridOperation.CreatDataGridView(columnsList, FleetAircraftTrendBarCollection, "FleetTrendAll");
                    }
                    i = 1;
                    ExportRadgridview.ElementExporting -= this.ElementExporting;
                    ExportRadgridview.ElementExporting += new EventHandler<GridViewElementExportingEventArgs>(ElementExporting);
        ...
        void ElementExporting(object sender, GridViewElementExportingEventArgs e)
        {
            ...
            if (e.Element == ExportElement.Cell &&
                e.Value != null)
            {
                if (i % 3 == 0 && i >= 6 && (sender as RadGridView).Name == "FleetTrendAll")
                {
                    e.Value = DateTime.Parse(e.Value.ToString()).AddMonths(1).AddDays(-1).ToString("yyyy/M/d");
                }
                else if (i % 4 == 3 && i >= 7 && (sender as RadGridView).Name == "SubFleetTrendAll")
                {
                    e.Value = DateTime.Parse(e.Value.ToString()).AddMonths(1).AddDays(-1).ToString("yyyy/M/d");
                }
            }
        ...
        private void OnExportGridView(object sender)
        {
            RadMenuItem menu = sender as RadMenuItem;
            IsContextMenuOpen = false;
            if (menu != null && menu.Header.ToString() == "导出数据" && planDetailGridview != null)
            {
                planDetailGridview.ElementExporting -= this.ElementExporting;
                planDetailGridview.ElementExporting += new EventHandler<GridViewElementExportingEventArgs>(ElementExporting);
                using (Stream stream = ImageAndGridOperation.DownloadDialogStream("文档文件(*.xls)|*.xls|文档文件(*.doc)|*.doc"))
                {
                    if (stream != null)
                    {
                        planDetailGridview.Export(stream, ImageAndGridOperation.SetGridViewExportOptions());
                    }
                }
            }
        }
        ...
        public void OnImportsSatisfied()
        {
            ...
            this.ExportGridViewCommand = new DelegateCommand<object>(this.OnExportGridView, this.CanExportGridView);//导出数据表数据
            SubscribeEvent();
