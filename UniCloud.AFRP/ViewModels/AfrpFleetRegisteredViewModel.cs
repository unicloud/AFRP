...
{
    [Export(typeof(AfrpFleetRegisteredViewModel))]
    [PartCreationPolicy(System.ComponentModel.Composition.CreationPolicy.Shared)]
    public class AfrpFleetRegisteredViewModel : NotificationObject, IPartImportsSatisfiedNotification, IConfirmNavigationRequest
    {
        public AfrpFleetRegisteredViewModel()
        {
        ...
        private readonly UniCloud.AFRP.Views.AfrpFleetRegisteredView _view = ServiceLocator.Current.GetInstance<UniCloud.AFRP.Views.AfrpFleetRegisteredView>();
            ...
            if (menu != null && menu.Header.ToString() == "导出源数据")
            {
                if (menu.Name == "MonthGridData")
                {
                    ...
                    using (Stream stream = ImageAndGridOperation.DownloadDialogStream("文档文件(*.xls)|*.xls|文档文件(*.doc)|*.doc"))
                    {
                        if (stream != null)
                        {
                            MonthExportRadgridview.Export(stream, ImageAndGridOperation.SetGridViewExportOptions());
                        }
                    }
                }
                else if (menu.Name == "YearGridData")
                {
                    ...
                    using (Stream stream = ImageAndGridOperation.DownloadDialogStream("文档文件(*.xls)|*.xls|文档文件(*.doc)|*.doc"))
                    {
                        if (stream != null)
                        {
                            YearExportRadgridview.Export(stream, ImageAndGridOperation.SetGridViewExportOptions());
                        }
                    }
                }
            }
