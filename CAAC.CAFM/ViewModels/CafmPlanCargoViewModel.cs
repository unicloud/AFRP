...
{
    [Export(typeof(CafmPlanCargoViewModel))]
    [PartCreationPolicy(System.ComponentModel.Composition.CreationPolicy.Shared)]
    public class CafmPlanCargoViewModel : NotificationObject, IPartImportsSatisfiedNotification, IConfirmNavigationRequest
    {
        public CafmPlanCargoViewModel()
        {
        ...
        private IEnumerable<PlanHistory> allCargoAircraftPlanHistories; //获取计划中的货机计划历史
        ...
        private readonly CAAC.CAFM.Views.CafmPlanCargoView _view = ServiceLocator.Current.GetInstance<CAAC.CAFM.Views.CafmPlanCargoView>();
        ...
        private void ShowtotalWindow(PieDataPoint selectedItem)
        {
            if (selectedItem != null)
            {
                AircraftStatistic selAc = selectedItem.DataItem as AircraftStatistic;
                //运营的飞机
                IEnumerable<Aircraft> operationAircrafts = ViewAircraft.Where(p => p.Airlines.ShortName == selAc.AirlineName);
                ...
                if (this.ViewPlan != null && allCargoAircraftPlanHistories != null)
                {
                    //获取选择航空公司的计划历史 
                    IEnumerable<PlanHistory> airlinePlanHistory = allCargoAircraftPlanHistories.Where(p => p.Plan.Airlines.ShortName == selAc.AirlineName && p.Annual.Year <= performYear).OrderBy(ob => ob.Annual.Year);
        ...
        private void ShowplanHistoryWindow(PieDataPoint selectedItem)
        {
            if (selectedItem != null)
            {
                AircraftCargoStatisticByYear selAc = selectedItem.DataItem as AircraftCargoStatisticByYear;
                if (this.ViewPlan != null && allCargoAircraftPlanHistories != null)
                {
                    //获取选择航空公司的计划历史 
                    IEnumerable<PlanHistory> airlinePlanHistory = allCargoAircraftPlanHistories.Where(p =>
                        p.Plan.Airlines.ShortName == selAc.AirlineName &&
                        p.AircraftType.AircraftCategory.Regional == selAc.CargoType &&
                        p.Annual.Year == selAc.Year);
                    //子窗体的标题
                    planHistoryWindow.Header = selAc.AirlineName +
                        selAc.Year + "年" + selAc.CargoType + "净增：" +
        ...
        public void InitializeAirlinePlanStatistics()
        {
            ...
            else
            {
                ...
                //if (this.ViewPlan.Where(p => p.CargoAircraftPlanHistories != null) != null && this.ViewPlan.Where(p => p.CargoAircraftPlanHistories != null).SelectMany(p => p.CargoAircraftPlanHistories) != null)
                if (allCargoAircraftPlanHistories != null)
                {

                    List<CargoPlanStatistics> alllPlanhistory = new List<CargoPlanStatistics>();
                    //统计基于年度的客机计划
                    var airlinePlanSatisfiedList = allCargoAircraftPlanHistories.OrderBy(p => p.Annual.Year).GroupBy(gb => gb.Annual.Year)
                       .Select(p => new CargoPlanStatistics
                       {
                           Year = p.Key,
                           LargeCargoAircraftNumber = p.Where(w => w.AircraftType.AircraftCategory.Regional == AppConfig.largeCargo) == null ? 0 : p.Where(w => w.AircraftType.AircraftCategory.Regional == AppConfig.largeCargo).Sum(m => m.DeltaCargo),
                           MidsizeCargoAircraftNumber = p.Where(w => w.AircraftType.AircraftCategory.Regional == AppConfig.midsizeCargo) == null ? 0 : p.Where(w => w.AircraftType.AircraftCategory.Regional == AppConfig.midsizeCargo).Sum(m => m.DeltaCargo),
                    ...
                    airlinePlanSatisfiedList.ForEach
                        (
                          air =>
                          {
                              CargoPlanStatistics aps = new CargoPlanStatistics();
                              aps.TotalName = "总飞机数";
                              aps.LargeName = this.LargeCargoLegendLabel;
                              aps.MidsizeName = this.MidsizeCargoLegendLabel;

                              aps.TotalColor = colordictionary[aps.TotalName];
                              aps.LargeColor = colordictionary[aps.LargeName];
                              aps.MidsizeColor = colordictionary[aps.MidsizeName];

                              aps.IncreaseLargeCargoNumber = air.LargeCargoAircraftNumber;
                              aps.IncreaseLargeToolTip = aps.LargeName + ":" + aps.IncreaseLargeCargoNumber + "架";

                              aps.IncreaseMidsizeCargoNumber = air.MidsizeCargoAircraftNumber;
                              aps.IncreaseMidsizeToolTip = aps.MidsizeName + ":" + aps.IncreaseMidsizeCargoNumber + "架";

                              aps.IncreaseTotalAircraft = aps.IncreaseLargeCargoNumber + aps.IncreaseMidsizeCargoNumber;
                              aps.IncreaseTotalToolTip = aps.TotalName + ":" + aps.IncreaseTotalAircraft + "架";


                              aps.LargeCargoAircraftNumber = airlinePlanSatisfiedList.Where(p => p.Year <= air.Year)
                                  .Sum(m => m.LargeCargoAircraftNumber)
                                  + ViewAircraft.Count(w => w.AircraftType.AircraftCategory.Regional == AppConfig.largeCargo);
                              aps.LargeToolTip = aps.LargeName + ":" + aps.LargeCargoAircraftNumber + "架";

                              aps.MidsizeCargoAircraftNumber = airlinePlanSatisfiedList.Where(p => p.Year <= air.Year)
                                  .Sum(m => m.MidsizeCargoAircraftNumber)
                                  + ViewAircraft.Count(w => w.AircraftType.AircraftCategory.Regional == AppConfig.midsizeCargo);
                              aps.MidsizeToolTip = aps.MidsizeName + ":" + aps.MidsizeCargoAircraftNumber + "架";

                              aps.TotalAircraft = aps.LargeCargoAircraftNumber + aps.MidsizeCargoAircraftNumber;
                              alllPlanhistory.Add(aps);
                              aps.TotalToolTip = aps.TotalName + ":" + aps.TotalAircraft + "架";
        ...
        public void InitializeCurrentOwerAircraft()
        {
            ...
            if (ViewAircraft != null && ViewAircraft.Count() > 0)
            {
                ...
                AircraftStatisticCollection = aircraftsByAirline.ToList();
        ...
        public void InitializeCurrentTotalAircraftCollection(int year)
        {
            ...
            if (this.ViewPlan != null && allCargoAircraftPlanHistories != null && allCargoAircraftPlanHistories.Where(p => p.Annual.Year <= year) != null)
            {

                //获取小于或等于今年计划飞机数的统计
                var aircraftStatisticList = allCargoAircraftPlanHistories.Where(p => p.Annual.Year <= year).GroupBy(gb => gb.Plan.Airlines.ShortName)
                   .Select(p => new AircraftStatistic
                   {
                       AirlineName = p.Key,
                       IncreaseAmount = p.Sum(m => m.DeltaCargo),
                       AircraftAmount = p.Sum(m => m.DeltaCargo)
                   }
                   ).ToList();
        ...
        #region Local 方法 InitializeAircraftCargo --获取当年航空公司货机类别的集合

        /// <summary>
        /// 获取当年航空公司类别的集合
        /// </summary>
        /// <param name="year">选择年度</param>
        /// <param name="cargoType">货机类型</param>
        public void InitializeAircraftCargo(int year, string cargoType)
        {
            AircraftCargoStatisticByYearCollection.Clear();

            //选择年度机型的统计，判断排除货机计划不存在
            if (this.ViewPlan != null && allCargoAircraftPlanHistories != null)
            {

                //基于年份、航空公司的每个年度每家航空公司飞机的统计
                var increaseAircraftStatistic = allCargoAircraftPlanHistories.Where(p =>
                    p.AircraftType.AircraftCategory.Regional == cargoType)
                    .GroupBy(gb => new { gb.Plan.Airlines.ShortName, gb.Annual.Year })
                   .Select(p => new AircraftCargoStatisticByYear
                   {
                ...
                if (increaseAircraftStatistic.Any())
                {
                    ...
                    AircraftCargoStatisticByYearCollection = increaseAircraftByYear;
                }
        ...
        public IEnumerable<Plan> ViewPlan
        {
            get
            {
                return this._service.EntityContainer.GetEntitySet<Plan>() == null ? null : this._service.EntityContainer.GetEntitySet<Plan>().Where(p => p.IsCurrentVersion == true && p.Annual.IsOpen == true && p.Airlines.IsValid == true && p.Airlines.MasterID == null);
            }
        }
        ...
        ///客机计划集合
        /// </summary>
        public IEnumerable<Plan> ViewCargoPlan
        {
            get
            {
                return this._service.EntityContainer.GetEntitySet<Plan>()
                    .Where(p => p.IsCurrentVersion == true && p.Annual.IsOpen == true && p.Airlines.IsValid == true && p.Airlines.MasterID == null && p.CargoAircraftPlanHistories != null && p.CargoAircraftPlanHistories.Count > 0);
            }
        }
        ...
        private void LoadPlan()
        {
            ...
            this._service.LoadPlan(new QueryBuilder<Plan>(), lo =>
            {
                ...
                this.IsBusy = this.IsBusyPlan || this.IsBusyAircraft || IsBusyXmlConfig;
                if (lo.Error != null)
                {
                    // 处理加载失败
                ...
                else
                {
                    ...
                    this.RaisePropertyChanged(() => this.ViewCargoPlan);
                    //获取计划中的货机计划历史
                    if (this.ViewPlan != null)
                    {
                        allCargoAircraftPlanHistories = this.ViewPlan.Where(p => p.CargoAircraftPlanHistories != null) == null ? null : this.ViewPlan.Where(p => p.CargoAircraftPlanHistories != null).SelectMany(p => p.CargoAircraftPlanHistories);
                    }
                    InitializeAirlinePlanStatistics();
                    SetRadCartesianChartColor();
                    GetNoRequstAirline();
        ...
        public IEnumerable<Aircraft> ViewAircraft
        {
            get
            {
                return this._service.EntityContainer.GetEntitySet<Aircraft>().Where(p => p.IsOperation == true && p.AircraftType.AircraftCategory.Category == "货机"); ;
            }
        }
        ...
        private string _aircraftTypeConent = "所选年度货机类型分布";
        /// <summary>
        /// 飞机机型饼状图的标题
        /// </summary>
        ...
        private string _totalAirlineContent = "所选年度货机分布";
        /// <summary>
        ///航空公司选择年度飞机饼状图的标题
        /// </summary>
        ...
        private List<CargoPlanStatistics> _airlinePlanSatisfiedCollection = new List<CargoPlanStatistics>();
        /// <summary>
        /// 各航空公司根据年份统计的计划集合
        /// </summary>
        public List<CargoPlanStatistics> AirlinePlanSatisfiedCollection
        {
        ...
        #region ViewModel 属性 AircraftCargoStatisticByYearCollection --选择年份机型的统计集合

        private List<AircraftCargoStatisticByYear> _aircraftCargoStatisticByYearCollection = new List<AircraftCargoStatisticByYear>();
        /// <summary>
        /// 选择年份机型的统计集合
        /// </summary>
        public List<AircraftCargoStatisticByYear> AircraftCargoStatisticByYearCollection
        {
            get { return _aircraftCargoStatisticByYearCollection; }
            set
            {

                if (AircraftCargoStatisticByYearCollection != value)
                {
                    _aircraftCargoStatisticByYearCollection = value;
                    if (value.Count == 0)
                    {
                        AircraftTypeNoDataString = "加载完成，数据为空";
                    }
                    this.RaisePropertyChanged(() => this.AircraftCargoStatisticByYearCollection);
                    SetPieMark2(AircraftCargoStatisticByYearCollection, this.LoadPieGrid);
                }
            }
        }
        #endregion
        ...
        #region ViewModel 属性 LargeCargoLegendLabel --大型货机图例标题

        /// <summary>
        ///大型货机图例标题
        /// </summary>
        public string LargeCargoLegendLabel
        {
            get
            {
                return AppConfig.largeCargo;
            }
        }
        #endregion

        #region ViewModel 属性 MidsizeCargoLegendLabel --中型货机图例标题

        /// <summary>
        ///中型货机图例标题
        /// </summary>
        public string MidsizeCargoLegendLabel
        {
            get
            {
                return AppConfig.midsizeCargo;
            }
        }
        #endregion
        ...
        private void OnManageFlagPlan(object obj)
        {
            ...
            //    if (sm.Error != null)
            //    {
            //        EventAggregatorRepository.EventAggregator.GetEvent<OprationMessageEvent>().Publish(Tuple.Create(OperationMessageType.Fail, "标记失败,请检查！"));
            //    }
            //    else if (!sm.Cancelled)
            //    {
            //        EventAggregatorRepository.EventAggregator.GetEvent<OprationMessageEvent>().Publish(Tuple.Create(OperationMessageType.Success, "标记成功！"));
            //        EventAggregatorRepository.EventAggregator.GetEvent<SetRefreshEvent>().Publish(true);
            //    }
            //    _isAnnotation = true;
            //}, null);
            //}
        ...
        private void OnExport(object sender)
        {
            ...
            if (menu != null && menu.Header.ToString() == "导出源数据")
            {
                if (menu.Name == "LineGridData")
                {
                    ...
                    columnsList.Add("LargeCargoAircraftNumber", "计划大型货机数");
                    columnsList.Add("MidsizeCargoAircraftNumber", "计划中型货机数");
                    columnsList.Add("TotalAircraft", "当年度所有计划货机数");
                    ExportRadgridview = ImageAndGirdOperation.CreatDataGridView(columnsList, AirlinePlanSatisfiedCollection, "CargoPlan");
                ...
                else if (menu.Name == "BarGridData")
                {
                    ...
                    Dictionary<string, string> columnsList = new Dictionary<string, string>();
                    columnsList.Add("Year", "年份");
                    columnsList.Add("IncreaseLargeCargoNumber", "大型货机净增数");
                    columnsList.Add("IncreaseMidsizeCargoNumber", "中型货机净增数");
                    columnsList.Add("IncreaseTotalAircraft", "总飞机净增数");
                    ExportRadgridview = ImageAndGirdOperation.CreatDataGridView(columnsList, AirlinePlanSatisfiedCollection, "CargoPlan");
                }
                else if (menu.Name == "AircraftPieGridData")
                {
                    ...
                    columnsList.Add("TotalAircraftNumberPercent", "当前运营货机数占比（%）");
                    ExportRadgridview = ImageAndGirdOperation.CreatDataGridView(columnsList, AircraftStatisticCollection, "CargoPlan");
                }
                else if (menu.Name == "SeatPieGridData")
                {
                    ...
                    Dictionary<string, string> columnsList = new Dictionary<string, string>();
                    columnsList.Add("AirlineName", "航空公司");
                    columnsList.Add("AircraftAmount", "某年计划后总货机数");
                    columnsList.Add("TotalAircraftNumberPercent", "占比（%）");
                    ExportRadgridview = ImageAndGirdOperation.CreatDataGridView(columnsList, AircraftStatisticByYearCollection, "CargoPlan");
                }
                else if (menu.Name == "LoadPieGridData")
                {
                    if (AircraftCargoStatisticByYearCollection == null || AircraftCargoStatisticByYearCollection.Count() == 0)
                    {
                        return;
                    }
                    //创建RadGridView
                    Dictionary<string, string> columnsList = new Dictionary<string, string>();
                    columnsList.Add("AirlineName", "航空公司");
                    columnsList.Add("AircraftAmount", "某年某座级的净增货机数");
                    columnsList.Add("TotalAircraftNumberPercent", "占比（%）");
                    ExportRadgridview = ImageAndGirdOperation.CreatDataGridView(columnsList, AircraftCargoStatisticByYearCollection, "CargoPlan");
                }
        ...
        public void ChartSelectionBehavior_SelectionChanged(object sender, Telerik.Windows.Controls.ChartView.ChartSelectionChangedEventArgs e)
        {
            ...
            if (SelectedPoint != null)
            {
                ...
                if (performYear != selAc.Year)
                {
                    ...
                    TotalAirlineContent = "截止" + selAc.Year.ToString() + "年货机分布";

                }

                //当前点击的大型货机是柱状图
                if (name == "IncreaseLargeCargoNumber" || name == "LargeCargoAircraftNumber")
                {
                    InitializeAircraftCargo(selAc.Year, AppConfig.largeCargo);
                    AircraftTypeConent = selAc.Year + "年净增" + AppConfig.largeCargo + "分布";
                }
                //当前点击的是中型货机柱状图
                if (name == "IncreaseMidsizeCargoNumber" || name == "MidsizeCargoAircraftNumber")
                {
                    InitializeAircraftCargo(selAc.Year, AppConfig.midsizeCargo);
                    AircraftTypeConent = selAc.Year + "年净增" + AppConfig.midsizeCargo + "分布";
                }
                if (name == "TotalAircraft")
                {
                    AircraftCargoStatisticByYearCollection = new List<AircraftCargoStatisticByYear>();
                    AircraftTypeConent = "所选年度货机类型分布";
                }
            }
        }
        ...
        public void RadPieChartSelectionBehavior_SelectionChanged1(object sender, Telerik.Windows.Controls.ChartView.ChartSelectionChangedEventArgs e)
        {
            ...
            if (radchartbase.EmptyContent.ToString() == "当前运营货机分布")
            {
        ...
        public void RadPieChartSelectionBehavior_SelectionChanged2(object sender, Telerik.Windows.Controls.ChartView.ChartSelectionChangedEventArgs e)
        {
            ...
            {
                StackPanel childstackpanel = stackpanel.Children
                    .FirstOrDefault(p => ((p as StackPanel).Children[1] as TextBlock).Text == (SelectedPoint.DataItem as AircraftCargoStatisticByYear).AirlineName) as StackPanel;
                System.Windows.Shapes.Rectangle rectangle = childstackpanel.Children[0] as System.Windows.Shapes.Rectangle;
                rectangle.Width = 12;
                rectangle.Height = 12;
        ...
        /// </summary>
        /// <param name="ienumerable">饼图数据集合</param>
        private void SetPieMark2(IEnumerable<AircraftCargoStatisticByYear> ienumerable, Grid grid)
        {
            RadPieChart radpiechart = grid.Children[0] as RadPieChart;
            StackPanel stackpanel = (grid.Children[1] as ScrollViewer).Content as StackPanel;
        ...
        void PiePanel_MouseLeftButtonDown1(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            foreach (var item in radpiechart.Series[0].DataPoints)
            {
                ...
                if ((piedatapoint.DataItem as AircraftStatistic).AirlineName == shortname)
                {
                    ...
                    {
                        if (radpiechart.EmptyContent.ToString() == "当前运营货机分布")
                        {
        ...
        void PiePanel_MouseLeftButtonDown2(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            {
                Telerik.Charting.PieDataPoint piedatapoint = item as Telerik.Charting.PieDataPoint;
                if ((piedatapoint.DataItem as AircraftCargoStatisticByYear).AirlineName == shortname)
                {
        ...
        private void SetRadCartesianChartColor()
        {
            ...
            foreach (var item in ((TrendGrid.Children[1] as RadCartesianChart).Resources["additionalVerticalAxis"] as AxisCollection))
            {
                ...
                {
                    linearaxis.ElementBrush = new SolidColorBrush(commonmethod.GetColor(colordictionary["中型货机"]));
                }
            }
            //控制折线趋势图的线条颜色
        ...
        public void OnImportsSatisfied()
        {
            ...
            InitalizerRadWindows(totalPlanHistoryWindow, "totalPlanHistoryWindow", 693);
