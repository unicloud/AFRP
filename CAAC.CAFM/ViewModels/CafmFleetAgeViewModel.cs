...
{
    [Export(typeof(CafmFleetAgeViewModel))]
    [PartCreationPolicy(System.ComponentModel.Composition.CreationPolicy.Shared)]
    public class CafmFleetAgeViewModel : NotificationObject, IPartImportsSatisfiedNotification, IConfirmNavigationRequest
    {
        public CafmFleetAgeViewModel()
        {
        ...
        CAAC.CAFM.Views.CafmFleetAgeView _view = ServiceLocator.Current.GetInstance<CAAC.CAFM.Views.CafmFleetAgeView>();
        ...
        public string SelectedTime
        {
            ...
            set
            {
                ...
                if (SelectedTime != value)
                {
                    ...
                    {
                        SelectedTimeAge = "所选时间和类型的机龄分布图";
                    }
                    else
                    {
                        string type = " 座级：";
                        if (this.AgeSelectedIndex == 1)
                        {
                            type = " 机型：";
                        }
                        else if (this.AgeSelectedIndex == 2)
                        {
                            type = " 航空公司：";
                        }
                        SelectedTimeAge = SelectedTime + type + SelectedType + "的机龄分布图";
                    }
                }
            }
        }
        ...
        private string _SelectedTimeAge = "所选时间和类型的机龄分布图";
        /// <summary>
        /// 机龄饼图的标识提示
        /// </summary>
        ...
        #region ViewModel 属性 AgeSelectedIndex --机龄的分类方式

        private int _AgeSelectedIndex = 0;
        /// <summary>
        /// 机龄的分类方式
        /// </summary>
        public int AgeSelectedIndex
        {
            get { return _AgeSelectedIndex; }
            set
            {

                if (AgeSelectedIndex != value)
                {
                    _AgeSelectedIndex = value;
                    CreateFleetAgeTrendCollection();
                    if (AgeSelectedIndex == 0)
                    {
                        (this.LineGrid.Children[0] as RadCartesianChart).TooltipTemplate = this._view.Resources["RegionalTooltipTemplate"] as DataTemplate;
                    }
                    else if (AgeSelectedIndex == 1)
                    {
                        (this.LineGrid.Children[0] as RadCartesianChart).TooltipTemplate = this._view.Resources["TypeTooltipTemplate"] as DataTemplate;
                    }
                    else if (AgeSelectedIndex == 2)
                    {
                        (this.LineGrid.Children[0] as RadCartesianChart).TooltipTemplate = this._view.Resources["AirlinesTooltipTemplate"] as DataTemplate;
                    }
                    this.RaisePropertyChanged(() => this.AgeSelectedIndex);
        ...
        {
            CAAC.CAFM.Views.CafmAgeDeployView window = ServiceLocator.Current.GetInstance<CAAC.CAFM.Views.CafmAgeDeployView>();
        ...
        private void OnExport(object sender)
        {
            ...
            if (menu != null && menu.Header.ToString() == "导出源数据")
            {
                if (menu.Name == "LineGridData")
                {
                    ...
                    if (AgeSelectedIndex == 1)
                    {
                        columnsList.Add("AircraftType", "机型");
                    }
                    if (AgeSelectedIndex == 2)
                    {
                        columnsList.Add("AircraftType", "航空公司");
                    }
                    columnsList.Add("AverageAge", "平均机龄");
                    ExportRadgridview = ImageAndGirdOperation.CreatDataGridView(columnsList, FleetAgeTrendCollection, "FleetAge");
                ...
                else if (menu.Name == "AgePieGridData")
                {
                    ...
                    //创建RadGridView
                    Dictionary<string, string> columnsList = new Dictionary<string, string>();
                    columnsList.Add("AgeGroup", "机龄区间");
                    columnsList.Add("GroupCount", "对应的飞机数");
                    ExportRadgridview = ImageAndGirdOperation.CreatDataGridView(columnsList, FleetAgeCollection, "PieFleetAge");
                }
                i = 1;
        ...
        void ElementExporting(object sender, GridViewElementExportingEventArgs e)
        {
            ...
            if (e.Element == ExportElement.Cell &&
               e.Value != null)
            {
                if (i % 4 == 3 && i >= 7 && (sender as RadGridView).Name == "FleetAge")
                {
